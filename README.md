...
The build scripts (powered by [tc-build](https://github.com/ClangBuiltLinux/tc-build)) behind this can be found [here](https://github.com/TsingPeng03/tc-build).

## Host compatibility

This toolchain is built on Ubuntu 20.04 LTS, which uses glibc 2.30. Compatibility with older distributions cannot be guaranteed. Other libc implementations (such as musl) are not supported.
...
```bash
export PATH="$HOME/toolchains/TsingClang/bin:$PATH"
```
